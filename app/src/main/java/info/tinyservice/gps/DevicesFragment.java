...
import info.tinyservice.gps.adapter.DeviceListAdapter;
            ...
            public void onServiceReady(OkHttpClient client, Retrofit retrofit, WebService service) {
                service.getDevices().enqueue(new WebServiceCallback<List<Device>>(getContext()) {
                    ...
                    public void onSuccess(Response<List<Device>> response) {
                        if(getActivity() != null){
                            DeviceListAdapter adapter = new DeviceListAdapter(DevicesFragment.this.getContext(), R.layout.device_item_list, response.body());
//                            setListAdapter(new ArrayAdapter<>(getContext(), R.layout.list_item, android.R.id.text1, response.body()));
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (activity != null) {
            ...
            activity.setResult(MapsActivity.RESULT_SUCCESS, new Intent().putExtra(EXTRA_DEVICE_ID, device.getId()));
            activity.finish();
        }
    }
